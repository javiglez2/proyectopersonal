// ==========================================
// UEQO - mapa.js (v3, con JWT)
// ==========================================

// Guarda: si no hay sesión, al login
if (!window.UEQO?.requireLogin()) throw new Error('No autenticado');

const usuarioID = window.UEQO.getUserId();
const nombreUsuario = window.UEQO.getUserNombre() || 'Usuario';
let mapa;
...
function urlAvatarFallback(nombre, bg = '1a2e25', color = '4ade80') {
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(nombre || '?')}&background=${bg}&color=${color}`;
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (avatarDisplay) {
        ...
        window.UEQO.apiFetchJSON(`/api/usuarios/${usuarioID}`)
...
async function cargarViajes() {
    ...
    try {
        todosLosViajes = await window.UEQO.apiFetchJSON('/api/viajes');
        todosLosViajes.sort((a, b) => new Date(a.fecha_hora_salida) - new Date(b.fecha_hora_salida));
        aplicarFiltros();
    } catch (e) {
        if (e?.status === 401) return; // Ya redirige UEQO.apiFetch
    ...
    viajesFiltrados.forEach(v => {
        ...
        div.innerHTML = `
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:12px;">
                ...
                <div style="display:flex; gap:8px; align-items:center;">${btnContactar}${btnHTML}</div>
            </div>
        `;
...
async function cargarMisViajes() {
    ...
    try {
        const viajes = await window.UEQO.apiFetchJSON(`/api/mis-viajes/${usuarioID}`);
        contenedor.innerHTML = '';
                ...
                ? v.reservas.map(r => {
                    ...
                    if (esConductor) {
                        return `<span style="background:#dbeafe; color:#1e40af; padding:4px 10px; border-radius:12px; font-size:12px; border:1px solid #bfdbfe; display:inline-flex; align-items:center; gap:6px; font-weight:bold;">
                            ...
                            <button onclick="window.location.href='chat.html'" style="background:none; border:none; cursor:pointer; padding:0; font-size:15px;" title="Ir a chats"></button>
                        </span>`;
                    } else {
...
window.borrarViaje = async function (idViaje) {
    ...
    if (result.isConfirmed) {
        ...
        try {
            await window.UEQO.apiFetchJSON(`/api/viajes/${idViaje}`, { method: 'DELETE' });
            await Swal.fire("Eliminado", "El viaje ha sido borrado.", "success");
            await cargarViajes();
            await cargarMisViajes();
        } catch (e) {
            if (e?.status === 401) return;
            Swal.fire("Error", e?.message || "No se pudo borrar el viaje", "error");
        }
    }
};
...
async function unirseViaje(idViaje, evento, boton) {
    ...
    try {
        // 🔑 Ya NO mandamos id_pasajero — el backend lo saca del token
        await window.UEQO.apiFetchJSON('/api/reservar', {
            method: 'POST',
        ...
        if (e?.status === 401) return;
        Swal.fire("Error", e?.message || "No se pudo unir", "error");
...
async function enviarViajeAlBack() {
    ...
    try {
        ...
        // 🔑 Ya NO mandamos id_conductor — el backend lo saca del token
        const datosViaje = {
        ...
        await window.UEQO.apiFetchJSON('/api/crear-viaje', {
            method: 'POST',
...
function hacerArrastrable(elmnt, handle) {
    ...
    function elementDrag(e) { e.preventDefault(); p1 = p3 - e.clientX; p2 = p4 - e.clientY; p3 = e.clientX; p4 = e.clientY; aplicarMovimiento(); }
    function closeDragElement() { document.onmouseup = null; document.onmousemove = null; }
    function dragTouchStart(e) { if (e.target.tagName === 'BUTTON') return; const touch = e.touches[0]; p3 = touch.clientX; p4 = touch.clientY; document.addEventListener('touchmove', dragTouchMove, { passive: false }); document.addEventListener('touchend', closeTouchDrag); }
    function dragTouchMove(e) { const touch = e.touches[0]; p1 = p3 - touch.clientX; p2 = p4 - touch.clientY; p3 = touch.clientX; p4 = touch.clientY; aplicarMovimiento(); }
    function closeTouchDrag() { document.removeEventListener('touchmove', dragTouchMove); document.removeEventListener('touchend', closeTouchDrag); }
...
// ==========================================
// NOTIFICACIONES GLOBAL + POLLING CON PAUSA
// ==========================================
async function comprobarNotificacionesGlobales() {
    ...
    try {
        ...
        let noLeidos = 0;
        try {
            const privados = await window.UEQO.apiFetchJSON(`/api/inbox/${usuarioID}`);
            privados.forEach(p => {
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) pararPollingNotificaciones();
    else { comprobarNotificacionesGlobales(); iniciarPollingNotificaciones(); }
});
